...
export function getTechniquesGroupedByCategory(): {
  category: TechniqueCategory;
  color: string;
  icon: string;
...
export function getTechniquesGroupedByType(): {
  type: TechniqueType;
  color: string;
  techniques: TechniqueMetadata[];
}[] {
  return TECHNIQUE_TYPE_ORDER.map((type) => ({
    ...
    techniques: TECHNIQUE_METADATA.filter(
      (t) => t.techniqueType === type && isTechniqueLessonVisible(t),
