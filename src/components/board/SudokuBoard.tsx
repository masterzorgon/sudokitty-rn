...
export const SudokuBoard = memo(
  ({
    ...
    const remeasureAndSelect = useCallback(
      (pageX: number, pageY: number) => {
        ...
        {Array.from({ length: BOARD_SIZE }, (_, row) => (
          <View key={row} style={styles.row}>
            {Array.from({ length: BOARD_SIZE }, (_, col) => {
              ...
              const isSecondaryHighlight = secondaryHighlightedCells?.has(key) ?? false;
              const isNumberHighlighted =
                highlightedNumber != null && cellData.value === highlightedNumber;
              ...
              const cellDelay = (row + col) * startGameAnimations.cellCascade.delayPerCell;
