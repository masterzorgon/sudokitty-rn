...
function GradientLayer({
  ...
  const positioning = useMemo(
    () => (reverse ? { bottom: 0, top: undefined as undefined } : { top: 0, bottom: undefined as undefined }),
    [reverse],
  );
  const layerStyle = useMemo(
    () => [styles.layer, { height, opacity }, positioning] as const,
    [height, opacity, positioning],
  );

  return (
    <>
      <LinearGradient
        key={`grad-${reverse}-${intensity}`}
