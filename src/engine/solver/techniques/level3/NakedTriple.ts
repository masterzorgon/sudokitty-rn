...
export class NakedTriple extends BaseTechnique {
  ...
  private findNakedTripleInUnit(grid: CandidateGridInterface, unit: Unit): TechniqueResult | null {
    // Get all empty cells in the unit with 2 or 3 candidates
    const eligibleCells: Position[] = grid.findEmptyCells(unit).filter((pos) => {
