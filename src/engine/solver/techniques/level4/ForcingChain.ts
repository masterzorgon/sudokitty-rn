...
export class ForcingChain extends BaseTechnique {
  ...
  private propagate(
    ...
    const queue: Implication[] = [{ type: 'place', row: startRow, col: startCol, value: startValue }];
    let depth = 0;
    ...
    while (queue.length > 0 && depth < MAX_DEPTH && !contradiction) {
      ...
      if (impl.type === 'place') {
        ...
        if (valueState[impl.row][impl.col] !== null && valueState[impl.row][impl.col] !== impl.value) {
          contradiction = true;
          break;
        }
        ...
        if (!candidateState[impl.row][impl.col].has(impl.value) && valueState[impl.row][impl.col] === null) {
          contradiction = true;
          break;
        }
  ...
  private findCommonPlacements(
    ...
    const common: Array<{ row: number; col: number; value: number }> = [];
  ...
  private findCommonEliminations(
    results: PropagationResult[]
  ): Elimination[] {
    if (results.length < 2) return [];
